...
inline Expr* ParseExpr(Lex &lexer, bool close, int precedence)
{
    ...
    if(Get(lexer) == Lexer::ParenOpen){
        ...
        if(Get(lexer) != Lexer::ParenClose){
            Error("Expected ')' to close call operator", lexer[0]);
        }
        Eat(lexer); // )
    }
